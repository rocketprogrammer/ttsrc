...
from toontown.distributed import ParentMgr
...
class AIZoneDataObj:
    ...
    def getParentMgr(self):
        if not hasattr(self, '_parentMgr'):
            ...
            self._parentMgr.registerParent(ToontownGlobals.SPHidden, hidden)
            self._parentMgr.registerParent(ToontownGlobals.SPRender, self.getRender())
    ...
    def startCollTrav(self, respectPrevTransform = 1, cTravName = None):
        ...
        if cTravName not in self._collTravsStarted:
            ...
            taskMgr.add(self._doCollisions, self._getCTravTaskName(name=cTravName), priority=ToontownGlobals.AICollisionPriority, extraArgs=[self._zoneId])
