...
class ConveyorBelt(BasicEntities.NodePathEntity):
    ...
    def start(self):
        ...
        if ConveyorBelt.UseClipPlanes:
            ...
            cpa = ClipPlaneAttrib.make(ClipPlaneAttrib.OAdd, headClip, tailClip)
            self.beltNode.node().setAttrib(cpa)
            cpa = ClipPlaneAttrib.make(ClipPlaneAttrib.ORemove, headClip, tailClip)
            for tread in self.treads:
                tread.parentingNode.node().setAttrib(cpa)
    ...
    def stop(self):
        ...
        if ConveyorBelt.UseClipPlanes:
            ...
            self.node().clearAttrib(cpaType)
            base.localAvatar.node().clearAttrib(cpaType)
