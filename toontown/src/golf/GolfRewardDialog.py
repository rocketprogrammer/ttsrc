...
class GolfRewardDialog:
    ...
    def calcTrophyTextListForOnePlayer(self, avId):
        """Return a list of trophy strings that this player has won."""
        retval = []
    ...
    def calcCupTextListForAllPlayers(self, localAvId):
        """ Return a list of cup strings that players have won with this player displayed last."""
        retval = []
        ...
        if len(rewardCupList) > 0:
            for rewardText in rewardCupList:
                oneCupIval = Parallel(
                    ...
                    LerpColorScaleInterval( self.rewardLabel, 4, Vec4(1,1,1,0), startColorScale = Vec4(1,1,1,1), blendType = 'noBlend')
                    )
                retval.append( oneCupIval )

        # display random winner message
        if self.tieBreakWinner:
            ...
            if av:
                ...
                randomWinnerIval = Parallel(
                    Func( setRewardLabelText, rewardText),
                    LerpColorScaleInterval( self.rewardLabel, 7, Vec4(1,1,1,0), startColorScale = Vec4(1,1,1,1), blendType = 'noBlend')
                    )
        ...
        if len(rewardHoleList) > 0:
            for rewardText in rewardHoleList:
                oneHoleIval = Parallel(
                    Func( setRewardLabelText, rewardText),
                    LerpColorScaleInterval( self.rewardLabel, 8, Vec4(1,1,1,0), startColorScale = Vec4(1,1,1,1), blendType = 'easeIn')
                    )
