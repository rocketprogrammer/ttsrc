...
class DistributedTugOfWarGame(DistributedMinigame):
    ...
    def __init__(self, cr):
        ...
        # game type will be set by the AI, just give it a default for now
        ...
        # these are the indexes of the active buttons
        self.buttons = [0,1]
        ...
        # variables used to even out a lopsided (2 toons vs 1 toon) game
        self.handycap = 2.
        self.advantage = 1.

        # variables for the rope and it's texture
        self.tugRopes = []
        self.ropePts = []
        self.ropeTex = []

        # these hold information about the positions and state of the toons
        self.rightHandDict = {}
        self.posDict = {}
        self.hprDict = {}
        self.offsetDict = {}
        ...
        self.drinkPositions = []
        for k in range(4):
            self.drinkPositions.append(VBase3(-.2+.2*k, 16+2*k, 0.0))

        # Create a random number generator with the same seed on each client
    ...
    def load(self):
        ...
        self.powerMeter.setPos(0,0,.4)

        # initalize the power meter, so the players can see what it looks like
    ...
    def unload(self):
        ...
        # Delete things initialized in the load function
        del self.lt
        self.timer.destroy()
        del self.timer
    ...
    def offstage(self):
        ...
        # Stop all tracks that are playing
        if self.introTrack:
    ...
    def initCamera(self):
        ...
        #gameCamPos = VBase3(-1.14881, 2.15348, 1.68946)
        #gameCamHpr = VBase3(-2.03389, 4.34384, 0.00744159)

        camera.reparentTo(render)
        camera.setPosHpr(introPosHpr[self.sides[self.localAvId]][0], introPosHpr[self.sides[self.localAvId]][1])

        # lerp from from the intro pos/hpr to the normal game camera view
        lerpDur = 8
        self.introTrack = LerpPosHprInterval(camera, lerpDur,
                                             pos = gameCamPos,
                                             hpr = gameCamHpr,
                                             blendType = 'easeInOut',
    ...
    def sendGameType(self, index, suit):
        ...
        self.suitLevel = suit
        if suit == 1:
            self.suitType = 'pp' # pencil pusher
        elif suit == 2:
            self.suitType = 'dt' # double talker
        elif suit == 3:
    ...
    def setGameReady(self):
        ...
        # Figure out where to place toons.  The type of game (toon vs. toon, or toon vs. cog)
        # needs to be known by this point
    ...
    def setUpRopes(self, notTaut):
        ...
        # setting up the ropes for TOON_VS_TOON is a little different
        # than for TOON_VS_COG
        ...
        if self.numPlayers == 1:
            ...
            if self.gameType == TugOfWarGameGlobals.TOON_VS_COG:
                self.tugRopes[1].setup(3, ((self.rightHandDict[self.avIdList[1]], (0, 0, 0)),
                                           (self.rightHandDict[self.avIdList[1]], (0, 0, 0)),
            ...
            if self.gameType == TugOfWarGameGlobals.TOON_VS_COG:
                ...
                self.tugRopes[2].setup(3, ((self.rightHandDict[self.avIdList[2]], (0, 0, 0)),
                                           (self.rightHandDict[self.avIdList[2]], (0, 0, 0)),
    ...
    def initToons(self):
        ...
        for avId in self.avIdList: #self.remoteAvIdList:
            ...
            if toon:
                ...
                # make sure the animations are cached for this toon
                for anim in self.toonAnimNames:
                    toon.pose(anim, 0)

                # Find heights of toons when tugging so we can arrange them
                # in tallest to shortest order
    ...
    def calculatePositions(self):
        ...
        # initialize which side the avatar is on
    ...
    def arrangeByHeight(self, avIdList, order, iStart, iFin):
        for i in range(iStart, iFin+1):
            for j in range(i+1, iFin+1):
                if ((order == self.H_TO_L and
                     self.rightHandDict[avIdList[i]].getZ() < self.rightHandDict[avIdList[j]].getZ()) or
                    (order == self.L_TO_H and
                     self.rightHandDict[avIdList[i]].getZ() > self.rightHandDict[avIdList[j]].getZ())):
                    # swap
                    temp = avIdList[i]
    ...
    def enterWaitForGoSignal(self):
        ...
        # Show the power gauge
        self.powerMeter.show()

        # tell the server we're ready to tug
        self.sendUpdate("reportPlayerReady", [self.sides[self.localAvId]])
        self.roundText.show()

        # start the timeout in case we don't get sent a go signal
    ...
    def enterTug(self):
        ...
        # start the timeout in case we don't get sent the stop signal from the AI
    ...
    def __updateIdealRateTask(self, task):
        ...
        if self.nextRateIndex < len(self.targetRateList):
            # if this is the last of the rate changes, make it so
            # force is decided by how fast you are pressing, not how
            # close to the correct rate you are
    ...
    def sendStopSignal(self, winners, losers, tieers):
        ...
        # we might get the stop signal before we ever started
        # or we might have already cleaned up
        ...
        if self.suit:
            if self.suitId in winners:
                ...
                # oops, the suit fell in while celebrating(only happens sometimes)
                ...
                if randInt < 3:
                    ...
                    # the suit will tilt forward while flailing
                    startHpr = self.suit.getHpr()
                    destHpr = startHpr + VBase3(0,0,-30)
                    oopsTrack = Sequence(Parallel(Func(self.suit.play, 'flail', None, 26, 38),
                                                  LerpHprInterval(self.suit, .5, destHpr, startHpr=startHpr)),
                                         Parallel(Func(self.suit.play, 'slip-forward'),
                    ...
                    # taunting
                    reactSeq.append(Sequence(Func(self.suit.loop, 'victory'),
                                                   Wait(2.6),
    ...
    def createSuits(self):
        if self.gameType == TugOfWarGameGlobals.TOON_VS_COG:
            ...
            #self.suit.setLevel(self.suitLevel)
            #self.suit.showNametag3d()
            #self.suit.showNametag2d()
            # make sure the animations are cached for this suit
    ...
    def setSpeedGauge(self):
        ...
        if not self.allOutMode:
            ...
            if index > 1:
                bonus = max(1,index-1)
                index = 1
            color = (0, .75*index+.25*bonus, .75*(1-index), .5)
    ...
    def throwInWater(self, avId=None):
        ...
        self.animTracks[animId] = Sequence(
            # Fall into water
            Parallel(ActorInterval(actor = loser, animName = 'slip-forward', duration = 2.0),
                     LerpPosInterval(loser, duration=2.0, pos=newPos),
                     # Splash
                     Sequence(Wait(1.0),
                              Parallel(Func(base.playSfx, self.sndHitWater),
    ...
    def computeForce(self, keyRate):
        ...
        if self.allOutMode == 1:
            F = .75*keyRate
        # otherwise, make force proportional to how close you are to key rate
        else:
            stdDev = .25*self.idealRate
            F = self.advantage * (self.rateMatchAward + 4+.4*self.idealRate) * math.pow(math.e, -math.pow(keyRate - self.idealRate, 2)/(2.0*math.pow(stdDev,2)))
        return F
    ...
    def __spawnUpdateRopeTask(self):
        ...
        taskMgr.add(self.__updateRopeTask,
    ...
    def __spawnMouseSpeedTask(self):
        ...
        taskMgr.add(self.__mouseSpeedTask,
    ...
    def __mouseSpeedTask(self, task):
        ...
        if self.mouseMode:
            ...
            # if last mouse position was on the left side, the new
            # position must be on the right in order to count it
            # as an arrow press, and vice versa
            if self.mouseSide == 0:
                if mx > dx:
                    self.mouseSide = 1
                    self.__releaseHandler(1)
