...
#
# Purpose: Object that stores player data and controls a local player
#===============================================================================
import math
...
class PartyCogActivityPlayer:
    ...
    def __init__(self, activity, toon, position, team):
        ...
        # Saved teamspot used for exiting toon positions.
    ...
    def exitsActivity(self):
        # Place toon outside the activity fence facing away from the activity

        # Adjust by spot so the exiting toons don't telefrag.
        offset = 0
        if self.teamSpot:
            offset = 4. * self.teamSpot
    ...
    def __showSplat(self, position):
        ...
        self.kaboomTrack = Parallel(
            SoundInterval(self.pieHitSound, node = self.toon, volume=1.0, cutOff = PartyGlobals.PARTY_COG_CUTOFF),
            Sequence(
...
class PartyCogActivityLocalPlayer(PartyCogActivityPlayer):
    ...
    def throwPie(self, piePower):
        ...
        if self.activity.getCurrentActivityTime() - self.throwPiePrevTime > THROW_PIE_LIMIT_TIME:
            assert(self.activity.notify.debug("LocalPlayer throwPie %d" % piePower))
