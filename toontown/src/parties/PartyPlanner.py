...
# Created: Aug 2008
#
# Purpose: The party planning gui, what you see when you travel to the party
#          grounds to plan a party.
#-------------------------------------------------------------------------------
...
class PartyPlanner(DirectFrame,FSM):
    """
    This class provides the GUI for planning a party. 
    """
    ...
    def __init__(self,doneEvent = None):
        ...
        #define the order the pages are displayed
        self.stateArray = ["Off","Welcome","PartyEditor","Guests","Date","Time","Invitation","Farewell" ]
    ...
    def calcAsapTime(self):
        """Calculate an Asap time with some padding in the future."""
        curServerTime = base.cr.toontownTimeManager.getCurServerDateTime()
        baseTime = curServerTime
        baseTime = baseTime.replace(baseTime.year, baseTime.month, baseTime.day, baseTime.hour, baseTime.minute,
    ...
    def enterInvitation(self,*args):       
        ...
        # Default to victory party theme invite if in victory party holiday.
    ...
    def load(self):
        """ 
        Create all of the UI elements needed for the Party Planner.
        """
        # Main frame for the planner        
        ...
        # welcome page shows a party planner and tells the guest what's up
        self.welcomePage = self._createWelcomePage()
        self.welcomePage.hide()

        # date page provides a calendar interface to select the date of the party
        self.datePage = self._createDatePage()
        self.datePage.hide()

        # time page provides a clock interface to select the time of the party
        self.timePage = self._createTimePage()
        self.timePage.hide()

        # guest page lets a user pick friends from their friends list to invite
        self.guestPage = self._createGuestPage()
        self.guestPage.hide()

        # partyEditor page should really take the user to the party editor
        self.partyEditorPage = self._createPartyEditorPage()
        self.partyEditorPage.hide()

        # review the party details and send out invitations
        self.invitationPage = self._createInvitationPage()
        self.invitationPage.hide()

        # Give the user feedback about what happened and say goodbye
    ...
    def __createNametag(self, parent):
        if self.nametagGroup == None:
            ...
            # Make sure this nametag group does not show any 2d elements
            self.nametagGroup.getNametag2d().setContents(0)
            
            # Create a nametag to display the name
            ...
            self.nametagNP.setPosHprScale(nametagPos[0], 0, nametagPos[2], 
                                          0, 0, 0, 
                                          0.1, 1, 0.1)
            
            # Create another nametag to display the instructions as a chat bubble
            ...
            self.chatNP.setPosHprScale(chatPos[0], 0, chatPos[2],
                                       0, 0, 0,
                                       0.08, 1, 0.08)
    ...
    def _createWelcomePage(self):
        """
        The contents of the welcome page.  The page shows the party planner!
        """
        self.nametagGroup = None
        ...
        # Different animals require different head scales.
    ...
    def _createDatePage(self):
        """
        Loads the calendar for date picking.
        """
        page = DirectFrame(self.frame)
    ...
    def _dayClickCallback(self,calendarGuiDay):
        """ Called when a day is clicked, turn it red and make it the day of the party. """
    ...
    def alterPartyTime(self,year=None,month=None,day=None,hour=None,minute=None):
        """
        Set partyTime based on old partyTime values and any new ones passed in.
        """
    ...
    def positiveTime(self, type, amount):
        """
        Returns the current value for type of partyTime if amount is None and
        checks for negative values or values over the max value for type,
    ...
    def _createTimePage(self):
        """
        Loads the clock page for time picking.
        """
        page = DirectFrame(self.frame)
        ...
        self.timePageRecapLocalTimeLabel = DirectLabel(
            parent = page,
            relief = None,
            text = "%s%s" % (TTLocalizer.PartyPlannerTimeLocalTime, PartyUtils.formatDateTime(self.partyTime, inLocalTime=True)),
    ...
    def getTimeWidgets(self, page, type):
        """
        Returns a label, an up button, and down button and creates the callback
        for changing hours, minutes, and am/pm values.
        """
        if type == "ampm":
    ...
    def _createGuestPage(self):
        """ Internal method for creating the contents of the guests page. """
        page = DirectFrame(self.frame)
        ...
        if len(base.localAvatar.friendsList) == 0:
            # They have no friends, skip this step, make the party public, skip
            # invitations.
            self.noFriends = True
        else:
    ...
    def _createPartyEditorPage(self):
        ...
        self.partyGridBackground = DirectFrame(
            parent = page,
            relief = None,
            geom = self.gui.find("**/partyGrid_flat"),
        )
        ...
        # how many jellybeans the toon has in their bank indicator
    ...
    def _createInvitationPage(self):
        """ Internal method for creating the contents of the Invitation page. """
        ...
        self.selectedInviteThemeLabel = DirectLabel(
            ...
            text = "", # e.g.: "Birthday Theme (1/4)", set by self.setInviteTheme()
            text_scale = 0.06,
            textMayChange=True,
        )
    ...
    def close(self):
        ...
        if hasattr(self, "okWithGroundsGui"):
            # we can get here with an AI reset
    ...
    def __handleComplete(self):
        """
        this should save party planning information and send invites to guests
        """
        ...
        # This is a list of lists. Each item in the higher list represents one
        # activity. Each activity is a list of constructor arguments. See
        # ActivityBase.py for details.
    ...
    def __acceptExit(self):
        """
        close er down
        TODO: make sure any unloading is handled if necessary
        """
    ...
    def __nextItem(self):
        ...
        # Not : This logic depends on the order of self.stateArray
    ...
    def okChooseFutureTime(self):
        """Handle choosing ok on choose future time dialog."""
    ...
    def __moneyChange(self, newMoney):
        """Properly handle the ~rich magic word."""
