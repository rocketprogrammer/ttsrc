...
filelistPath = '$TOONTOWN/src/publish/filelist'
...
class Scrubber:
    def __init__(self):
        ...
        # supports decimal hashes.  For now, only the Mac publish uses
        # the new-style publish.

        self.ddbHexHashes = False
        if platform == 'OSX':
    ...
    def doWiseCommand(self):
        useBzip2 = False
        if platform == 'WIN32':
            basename = 'InstallLauncher.exe'
        elif platform == 'OSX':
            useBzip2 = True
            basename = 'Toontown_OSX.tar'
            if mode == 'test':
        ...
        if not cdmode:
            ...
            wiseInstallFile = Filename.expandFrom('$TOONTOWN/src/launcher/' + basename)
        else:
            webLdbName = Filename(self.persistDir, Filename('../launcherFileDb'))
            wiseInstallFile = Filename(self.persistDir, Filename('../' + basename))
        persistFilename = Filename(self.persistDir, Filename(basename))
        installFilename = Filename(self.installDir, Filename(basename))

        # read previous hashes from .ddb file, if it exists.  There
        # are two different formats for .ddb file, the older
        # launcherFileDb.ddb, and the newer bs.ddb.  We'll accept
        # either one.

        lfddbFilename = Filename(self.persistDir, Filename('launcherFileDb.ddb'))
        bsddbFilename = Filename(self.persistDir, Filename('bs.ddb'))

        if bsddbFilename.exists():
            ddbHashes = self.readDdbHashes(bsddbFilename, True)

        elif lfddbFilename.exists():
            ddbHashes = self.readDdbHashes(lfddbFilename, False)
        ...
        # write the InstallLauncher.exe file name and its size to progress
        installSize = wiseInstallFile.getFileSize()
        self.progressMap[wiseInstallFile.getBasename()] = installSize

        # Compute the md5 on the file
        md5 = HashVal()
        if (not md5.hashFile(wiseInstallFile)):
            self.notify.error("Unable to hash %s." % (wiseInstallFile.cStr()))

        # Does the InstallLauncher exist on persist directory
        persistFilenameExists = persistFilename.exists()
        if (persistFilenameExists):
            # Compute the md5 on the old file
            md5Old = HashVal()
            if (not md5Old.hashFile(persistFilename)):
                self.notify.error("Unable to hash %s." % (persistFilename.cStr()))

            # If the hash values are equal, no need to update the file
            if md5.eq(md5Old) and len(ddbHashes) > 0 and ddbHashes[0].eq(md5Old):
                self.notify.debug('%s has not changed.' % basename)
                return

        # add the new md5 as the first hash
        ddbHashes = [md5] + ddbHashes

        if persistFilenameExists:
            self.notify.info('%s has changed. Installing new version.' % (basename))
            self.notify.debug('  Old hash: %s' % (md5Old.asDec()))
            self.notify.debug('  New hash: %s' % (md5.asDec()))
            # Generate the patch file and then rename the new file into place.
            highVer = self.generatePatch(persistFilename, wiseInstallFile, md5)
        else:
            self.notify.info('installerScrubber: copying new version.')
            # just move the newly created InstallLauncher.exe to persist directory
            self.moveFile(wiseInstallFile, persistFilename)

        # Copy (or compress) the now-patched InstallLauncher.exe to
        # the install directory.
        if useBzip2:
            compFilename = Filename(installFilename.cStr() + '.bz2')
            self.compressFile(persistFilename, compFilename, useBzip2 = True)
            fileSize = compFilename.getFileSize()
        else:
            self.copyFile(persistFilename, installFilename)
            fileSize = persistFilename.getFileSize()

        if persistFilenameExists:
            # Now copy the patches to the install directory
            lastVersion = self.installerCopy(highVer, persistFilename,
                                             fileSize = fileSize,
                                             useBzip2 = useBzip2)

            # if deleted obsolete patches, then truncate those hash
            # values from ddb
            index = 0
            ddbHashes = ddbHashes[:lastVersion]

        # Rewrite launcherFileDb.ddb and bs.ddb
        self.writeDdbHashes(basename, ddbHashes, lfddbFilename, False)
        self.writeDdbHashes(basename, ddbHashes, bsddbFilename, True, fileSize)

        self.writeProgressFile()

        # Rewrite launcherFileDb to include the InstallLauncher.exe
        # line.  We only do this on the Windows build; probably, when
        # we migrate the Windows publish to the new launcher system,
        # we won't even do it there.
        if platform == 'WIN32':
            oldLauncherFile = open(webLdbName.toOsSpecific(), 'r')
            ...
            for i in range (0, len(lines)):
                ...
                if index > 0:
                    ...
                    if filename == basename:
                        del lines[i]
                        break
            lines.append(basename + ' ' + md5.asDec() + '\n')

            newLauncherFile = open(self.launcherFileDb.toOsSpecific(), 'w')
            self.notify.info('writing launcherFileDb = ' + self.launcherFileDb.cStr())
    ...
    def doCopyCommand(self):
        ...
        for mi in range(numMfiles):
            ...
            if self.phase2.match(mfname) == None:    # don't copy phase_2 files
