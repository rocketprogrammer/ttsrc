...
class Quest:
    ...
    def checkRecoveryItemHolderAndType(self, holder, holderType='type'):
        ...
        if holderType == 'type':
            holders = [
             Any, AnyFish] + list(SuitBattleGlobals.SuitAttributes.keys())
...
class CogTrackQuest(CogQuest):
    __module__ = __name__
    trackCodes = ['c', 'l', 'm', 's']
    trackNamesS = [TTLocalizer.BossbotS, TTLocalizer.LawbotS, TTLocalizer.CashbotS, TTLocalizer.SellbotS]
    trackNamesP = [
     TTLocalizer.BossbotP, TTLocalizer.LawbotP, TTLocalizer.CashbotP, TTLocalizer.SellbotP]
...
class SkelecogTrackQuest(CogTrackQuest, SkelecogQBase):
    __module__ = __name__
    trackNamesS = [TTLocalizer.BossbotSkelS, TTLocalizer.LawbotSkelS, TTLocalizer.CashbotSkelS, TTLocalizer.SellbotSkelS]
    trackNamesP = [
     TTLocalizer.BossbotSkelP, TTLocalizer.LawbotSkelP, TTLocalizer.CashbotSkelP, TTLocalizer.SellbotSkelP]
...
class FactoryQuest(LocationBasedQuest):
    __module__ = __name__
    factoryTypeNames = {FT_FullSuit: TTLocalizer.Cog, FT_Leg: TTLocalizer.FactoryTypeLeg, FT_Arm: TTLocalizer.FactoryTypeArm, FT_Torso: TTLocalizer.FactoryTypeTorso}
...
class DeliverItemQuest(Quest):
    ...
    def getSCStrings(self, toNpcId, progress):
        ...
        article = iDict[2]
        itemName = iDict[0]
        return [TTLocalizer.QuestsDeliverItemQuestSCString % {'article': article, 'itemName': itemName}] + getVisitSCStrings(toNpcId)
...
class RecoverItemQuest(LocationBasedQuest):
    ...
    def getProgressString(self, avatar, questDesc):
        if self.getCompletionStatus(avatar, questDesc) == COMPLETE:
            ...
            return TTLocalizer.QuestsRecoverItemQuestProgress % {'progress': questDesc[4], 'numItems': self.getNumItems()}
    ...
    def getSCStrings(self, toNpcId, progress):
        ...
        if progress >= self.getNumItems():
            ...
            if toNpcId == ToonHQ:
                ...
                strings = [TTLocalizer.QuestsRecoverItemQuestReturnToSCString % {'item': itemName, 'npcName': npcName}]
                if isInPlayground:
        ...
        if num == 1:
            ...
            itemName = TTLocalizer.QuestsItemNameAndNum % {'num': TTLocalizer.getLocalNum(num), 'name': ItemDict[item][1]}
        return TTLocalizer.QuestsRecoverItemQuestRecoverFromSCString % {'item': itemName, 'holder': holderName, 'loc': locName}
...
class TrackChoiceQuest(Quest):
    ...
    def getSCStrings(self, toNpcId, progress):
        ...
        trackAName = ToontownBattleGlobals.Tracks[trackA].capitalize()
        trackBName = ToontownBattleGlobals.Tracks[trackB].capitalize()
        return [TTLocalizer.QuestsTrackChoiceQuestSCString % {'trackA': trackAName, 'trackB': trackBName}, TTLocalizer.QuestsTrackChoiceQuestMaybeSCString % trackAName, TTLocalizer.QuestsTrackChoiceQuestMaybeSCString % trackBName] + getVisitSCStrings(toNpcId)
...
def getNpcLocationDialog(fromNpcId, toNpcId):
    ...
    paragraph = TTLocalizer.QuestsLocationParagraph % {'building': TTLocalizer.QuestsLocationBuilding % toNpcName, 'buildingName': toBuildingName, 'buildingVerb': TTLocalizer.QuestsLocationBuildingVerb, 'street': streetDesc}
    return (
...
def checkReward(questId, forked=0):
    ...
    if nextQuests is None:
        validRewards = list(RewardDict.keys()) + [Any, AnyCashbotSuitPart, OBSOLETE]
