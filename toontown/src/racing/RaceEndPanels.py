...
class RaceResultsPanel(DirectFrame):
    ...
    def __init__(self,numRacers,race, raceEndPanel, *args,**kwargs):
        ...
        self.tickets = {}
        self.race = race
        self.raceEndPanel = raceEndPanel #self.getParent() doesn't work
        ...
        for x in range(self.numRacers):
            ...
            self.entryList.append((pLabel,fFrame,nLabel,tLabel,wLabel,cpLabel,ncpLabel))
    ...
    def displayRacer(self,place,headFrame,name,time,qualify,tickets,bonus,trophies,circuitPoints,circuitTime):
        self.notify.debug( "displayRacer: place=%d name=%s tickets=%d bonus=%d" % (place,name,tickets,bonus))
        ...
        if len(name) >24:
            #TODO it's better to compute the length of the name as displayed
            #on screen, then adjust the text scale from that value
        ...
        if not circuitPoints == []:
            ...
            seq = Sequence(
                Wait(1),
                Parallel(LerpFunc(totalPointTicker, duration=1),
    ...
    def circuitFinished(self, placeFixup):
        ...
        for (oldPlace,newPlace) in placeFixup:
            if oldPlace != newPlace:
                ...
                shiftRacersSeq.append( Parallel(
                    Func(fixPlaceValue),
                    LerpPosInterval(self.rowFrame[oldPlace], 1, newPos)
                    ))

        self.circuitFinishSeq = Sequence(
            calcPointsSeq,
            shiftRacersSeq,
            )
        ...
        if (not len(self.race.circuitLoop) == 0):
            ...
            #so we know that the next race button is up, if he doesn't press it in 30 secs, do it for him
    ...
    def generateDisplaySequences(self,track,ticDeposit,ticWon,ticBonus,trophies, endOfCircuitRace = False):
        ...
        if ticBonus:
            ticketSeq.append(
                ...
                Func(self.bonusComponents[0].configure,text =
                     wrapStr( TTLocalizer.KartRace_RecordString % ( TTLocalizer.KartRecordStrings[bonusType],
        ...
        ticketSeq.append(
            ...
            Parallel(
            LerpFunc(ticketTicker,duration = (ticDeposit and [1] or [0])[0],extraArgs = [self.ticketComponents[0],0,ticDeposit]),
            LerpFunc(ticketTicker,duration = (ticDeposit and [1] or [0])[0],extraArgs = [self.ticketComponents[3],0,ticDeposit])
            ),
            ...
            Parallel(
            LerpFunc(ticketTicker,duration = (ticWon and [1] or [0])[0],extraArgs = [self.ticketComponents[1],0,ticWon]),
            LerpFunc(ticketTicker,duration = (ticWon and [1] or [0])[0],extraArgs = [self.ticketComponents[3],ticDeposit,ticDeposit+ticWon]),
            ),
            ...
            Parallel(
            LerpFunc(ticketTicker,duration = (origTicBonus and [1] or [0])[0],extraArgs = [self.ticketComponents[2],0,origTicBonus]),
            LerpFunc(ticketTicker,duration = (origTicBonus and [1] or [0])[0],extraArgs = [self.ticketComponents[3],ticDeposit+ticWon,ticDeposit+ticWon+origTicBonus]),
            ),
...
class RaceEndPanel(DirectFrame):
    ...
    def displayRacer(self,place,entryFee,qualify,winnings,track,bonus,trophies,headFrame,name,time,circuitPoints,circuitTime):
        ...
        # qualify = did the racer beat the qualifying time?
        # winnings = total number of tickets won
        # track = track id as defined in RaceGlobals
        # bonus = bonus amount in tickets
        # trophies = tuple of trophy ids that have been achieved by this race as defined in TTLocalizer
        # headFrame = nodepath to the geometry displayed on the face line
        # name = name of the racer
        # circuitPoints = array of points for this racer for each circuitRace
        # circuitTime = sum of all the times in the circuit
        # time it took to finish the race
        self.results.displayRacer(place,
                                  headFrame,
                                  name,
                                  time,
                                  qualify,
    ...
    def startWinningsPanel(self,entryFee,winnings,track,bonus = None,trophies = (), endOfCircuitRace = False):
        ...
        tSeq,wSeq =  self.winnings.generateDisplaySequences(track,
                                                            entryFee,
                                                            winnings,
                                                            bonus,
                                                            trophies,
        ...
        def showButton(s = self,w = wSeq):
            s.seq.finish()
            s.seq = Sequence(Func(self.closeButton.show),
                             wSeq)
            s.seq.loop()

        self.seq = Sequence(tSeq,
                            wSeq)
    ...
    def circuitFinished(self, placeFixup):
        # show the 'next race' or 'exit' button
        #self.seq.finish()
