...
# dynamically.  We do this by copying from the above lists.
allocatedIndexes = {}
...
def getFirstRoute(playground, area, doId):
    """ Return a starting point, destination point, and time to destination
    """
...
def __getCurrentPos(playground, area, doId):
    ...
    # We can only prevent duplicate destinations if there are any unused points
    if (len(unusedI) == 0):
...
def getNextPos(currentPos, playground, area, doId):
    ...
    # Make sure we're heading to a new spot
    nextPos = currentPos
