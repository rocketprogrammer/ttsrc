...
class InventoryPage(ShtikerPage.ShtikerPage):
    ...
    def __init__(self):
        """__init__(self)
        InventoryPage constructor: create a shtiker book page for the inventory
        """
    ...
    def clearTrackInfo(self, trackIndex):
        ...
        # have to allow for the possibility that we got a "within"
        # event for some other track before we got a "without" event
        # for the current track.  Thus, we only clear out the current
        # track info if it's still showing the track we think it is.
