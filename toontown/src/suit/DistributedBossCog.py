...
class DistributedBossCog(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    ...
    def localToonToSafeZone(self):
        ...
        place = self.cr.playGame.getPlace()
        place.fsm.request('teleportOut', [{'loader': ZoneUtil.getLoaderName(target_sz), 'where': ZoneUtil.getWhereName(target_sz, 1), 'how': 'teleportIn', 'hoodId': target_sz, 'zoneId': target_sz, 'shardId': None, 'avId': -1, 'battle': 1}])
        return

    def localToonDied(self):
        target_sz = ZoneUtil.getSafeZoneId(localAvatar.defaultZone)
        place = self.cr.playGame.getPlace()
        place.fsm.request('died', [{'loader': ZoneUtil.getLoaderName(target_sz), 'where': ZoneUtil.getWhereName(target_sz, 1), 'how': 'teleportIn', 'hoodId': target_sz, 'zoneId': target_sz, 'shardId': None, 'avId': -1, 'battle': 1}])
        return
    ...
    def zapLocalToon(self, attackCode, origin=None):
        ...
        self.sendUpdate('zapToon', [pos[0], pos[1], pos[2], hpr[0], hpr[1], hpr[2], bp2d[0], bp2d[1], attackCode, timestamp])
        self.__doZapToon(toon, fling=fling, shake=shake)
        return
    ...
    def showZapToon(self, toonId, x, y, z, h, p, r, attackCode, timestamp):
        ...
        toon = self.cr.doId2do.get(toonId)
        if toon:
            self.__doZapToon(toon, pos=pos, hpr=hpr, ts=ts, fling=fling)
        return

    def __doZapToon(self, toon, pos=None, hpr=None, ts=0, fling=1, shake=1):
    ...
    def setAttackCode(self, attackCode, avId=0):
        ...
        if attackCode == ToontownGlobals.BossCogDizzy:
            ...
            self.setDizzy(0)
            self.__doDirectedAttack(avId, attackCode)
    ...
    def loseCogSuits(self, toons, battleNode, camLoc, arrayOfObjs=False):
        ...
        for toon in toonArray:
            ...
            suitsOff.append(Sequence(Func(dustCloud.reparentTo, toon), Parallel(dustCloud.track, Sequence(Wait(0.3), Func(toon.takeOffSuit), Func(toon.sadEyes), Func(toon.blinkEyes), Func(toon.play, 'slip-backward'), Wait(0.7))), Func(dustCloud.detachNode)))

        seq.append(suitsOff)
        return seq

    def __doDirectedAttack(self, avId, attackCode):
