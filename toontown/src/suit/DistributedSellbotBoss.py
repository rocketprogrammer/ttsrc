...
class DistributedSellbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def makeIntroductionMovie(self, delayDeletes):
        ...
        dialogTrack = Track((0,
         ...
         34,
         Sequence(Func(self.clearChat), self.loseCogSuits(self.toonsA, self.battleANode, (0, 18, 5, -180, 0, 0)), self.loseCogSuits(self.toonsB, self.battleBNode, (0, 18, 5, -180, 0, 0)))), (
         37,
    ...
    def makeEndOfBattleMovie(self, hasLocalToon):
        ...
        if hasLocalToon:
            seq += [Func(camera.reparentTo, render), Func(camera.setPosHpr, self.cage, 0, -50, 0, 0, 0, 0), Func(localAvatar.setCameraFov, ToontownGlobals.CogHQCameraFov), Func(self.hide)]
        seq += [Wait(0.5),
         Parallel(self.cage.posInterval(1, self.cagePos[(self.cageIndex + 1)], blendType='easeInOut'), SoundInterval(self.cageLowerSfx, duration=1)), Func(self.cagedToon.nametag3d.setScale, 2),
         Func(self.cagedToon.setChatAbsolute, TTLocalizer.CagedToonDrop[self.cageIndex], CFSpeech), Wait(3), Func(self.cagedToon.nametag3d.setScale, 1), Func(self.cagedToon.clearChat)]
        if hasLocalToon:
            seq += [Func(self.show), Func(camera.reparentTo, localAvatar), Func(camera.setPos, localAvatar.cameraPositions[0][0]), Func(camera.setHpr, 0, 0, 0)]
        self.cageIndex += 1
        return seq
